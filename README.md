...
Таблица
| Тип пользователя | Метрика (количество на 1 пользователя в день)   | Action(число) | RPS      | RPS x3    | Объем (Кб) | Сеть (Кб/сек) | Сеть x3 (Кб/сек) |
| ---------------- | ------------------------------------------------ | ------------- | -------- | --------- | ----------- | ----------- | -------------- |
...
#### Итого суммарно на продавца и покупателя в день:

Сеть total = 8.84 Гб/сек.<br>
...
Основной датацентр можно разместить в центральном регионе, лучше в Техасе, так как это обеспечит лучшую задержку для пользователей как на востоке, так и на западе.
